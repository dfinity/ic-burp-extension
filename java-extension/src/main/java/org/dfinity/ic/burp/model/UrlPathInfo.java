...
public record UrlPathInfo(String canisterId, RequestType requestType) {
    ...
    private static final Pattern IC_API_V3_PATH_REGEX = Pattern.compile("/api/v3/canister/(?<cid>[^/]+)/(?<rtype>query|call|read_state)");
    private static final Pattern IC_API_V4_PATH_REGEX = Pattern.compile("/api/v4/canister/(?<cid>[^/]+)/(?<rtype>call)");

    public static Optional<UrlPathInfo> tryFrom(String path) {
        for (var pathRegex : List.of(IC_API_V2_PATH_REGEX, IC_API_V3_PATH_REGEX, IC_API_V4_PATH_REGEX)) {
